...
expect "$(bash "$CFG/statusline/spend-statusline.sh" --display 2>&1 | grep -c '^pace  *off')" 1
expect "$(bash "$CFG/statusline/spend-statusline.sh" --display 2>&1 | grep -c 'skipping')" 0
